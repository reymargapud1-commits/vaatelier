 ...
 * lesson as completed, AND passed every module quiz at least once. Does
 * NOT check feedback or whether a certificate row already exists - see
...
async function isTrackComplete(userId: string, courseId: string, trackId: string): Promise<boolean> {
  ...
  if (trackLessonIds.length > 0) {
    ...
    if (completed.length < trackLessonIds.length) return false;
  }
...
/**
 * Whether the student has already rated this track (1-5 stars).
 */
export async function hasSubmittedFeedback(userId: string, courseId: string, trackId: string): Promise<boolean> {
  const [row] = await db
    .select()
    .from(trackFeedback)
    .where(
      and(
        ...
        eq(trackFeedback.track, trackId)
      )
    )
    .limit(1);
  return !!row;
 ...
 * modules only:
 *   1. Marked every lesson as completed,
 *   2. Passed every module quiz at least once, AND
 *   3. Rated the training (see trackFeedback / hasSubmittedFeedback).
...
export async function checkAndIssueCertificate(userId: string, courseId: string, trackId: string) {
  ...
  const complete = await isTrackComplete(userId, courseId, trackId);
  if (!complete) return null;

  const rated = await hasSubmittedFeedback(userId, courseId, trackId);
...
/**
 * Tracks that are fully complete (lessons + quizzes) but do not have a
 * certificate yet because the student hasn't rated the training. Used to
 * show the "Congratulations - just rate the training" prompt right after a
 * quiz is passed, and as a persistent CTA on the certificates page.
 */
export async function getTracksAwaitingFeedback(userId: string, courseId: string) {
  ...
  for (const track of CERTIFICATE_TRACKS) {
    const [existing] = await db
      .select()
      .from(certificates)
      .where(and(eq(certificates.userId, userId), eq(certificates.courseId, courseId), eq(certificates.track, track.id)))
      .limit(1);
    if (existing) continue;

    const complete = await isTrackComplete(userId, courseId, track.id);
    if (!complete) continue;

    const rated = await hasSubmittedFeedback(userId, courseId, track.id);
    if (rated) continue;
...
export async function getTrackProgress(userId: string, courseId: string) {
  ...
  return CERTIFICATE_TRACKS.map((track) => {
    ...
    const issued = issuedTrackIds.has(track.id);
    const eligible = totalLessons > 0 && completedCount === totalLessons && allQuizzesPassed;
    return {
      track,
      percent,
      completedCount,
      ...
      eligible,
      issued,
      awaitingFeedback: eligible && !issued && !ratedTrackIds.has(track.id),
    };
  });
}
