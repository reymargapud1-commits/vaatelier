...
import { generateBillingStatementPdf } from "@/lib/billing-pdf";

/** Streams the Billing Statement PDF (personal client -> their customer) for one batch. */
export async function GET(req: Request, { params }: { params: { batchId: string } }) {
  ...
  const pdfBytes = await generateBillingStatementPdf(batch, client, customer, trips);

  return new NextResponse(new Uint8Array(pdfBytes), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `inline; filename="Billing-Statement-${batch.bsNumber}.pdf"`,
    },
  });
}
